        ...
        mainMartor.each(function(i, obj) {
            ...
            /*var emojiWordCompleter = {
                getCompletions: function(editor, session, pos, prefix, callback) {
                    ...
                    console.log("about to complete emoji")
                    if (lastToken[0] == ':') {
            ...
            var emojiWordCompleter = {
                getCompletions: function(editor, session, pos, prefix, callback) {
                    if(called ==false){
                      wordList = ['Choledocholithiasis','gallstone','with','data','support','vector','machine','algorithm','and','for','this','is', 'duct', 'stone', 'bileduct', 'form', 'gallbladder', 'people', 'called', 'abdomen',]; 
                    }
                    // from `atwho/emojis.min.js`
                    var obj = editor.getSession().getTokenAt(pos.row, pos.column.count);
                    var curTokens = obj.value.split(/\s+/);
                    //console.log("cur: "+ curTokens)
                    var lastToken = curTokens[curTokens.length-1];
                    lastToken = ":"+lastToken
                    //console.log("last: "+ lastToken)
                    var user_text = editor.getValue();
                    if(user_text.split(/\s+/).length > 30 && called == false){
                      called=true
                      var xhr = new XMLHttpRequest();   // new HttpRequest instance 
                      xhr.open("POST", "/get");
                      //console.log(user_text)
                      xhr.setRequestHeader("Content-Type", "application/json");
                      xhr.send(JSON.stringify({text:user_text}));
                      console.log("user_text sent"+user_text)
                      xhr.onreadystatechange = processRequest;
                      function processRequest(e) {
                          if (xhr.readyState == 4 && xhr.status == 200) {
                              console.log('getting dictionary')
                              wordList  = ['population', 'and', 'the', 'working', 'of', 'miniland', 'society']
